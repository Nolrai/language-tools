...
import Control.Exception (SomeException, handle, Exception (displayException), throwIO)
import Data.Bool (Bool (..), not)
import Data.ByteString as BS
import Data.Dictionary.Utils (composeDicts, reverseDict, (:=>), linesToDict)
...
checkForInvalidIpas ipas = do
  ...
  let invalidChars = Set.unions
          [ Set.filter (not . isIPAChar) (Set.fromList (T.unpack ipa))
          | ipa <- Set.toList ipas ]
  unless (Set.null invalidChars) $
    throwIO . userError
      $ "warning: found invalid IPA characters that may cause lexurgy to fail: "
